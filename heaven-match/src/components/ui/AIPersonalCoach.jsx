...
export const AICoachDemo = () => {
  // State for the user-selected question, defining the advice topic.
  const [selectedQuestion, setSelectedQuestion] = useState('How do I keep the spark alive?');
  // State to hold the array of generated advice tips (strings).
  ...
  /**
   * Asynchronously calls the Gemini API to fetch relationship advice.
   */
  const getAdvice = async () => {
    ...
    // Using a suitable model for fast, creative text generation.
    const apiUrl = `https://generativelanguage.googleapis.com/v1beta/models/gemini-2.0-flash-exp:generateContent?key=${apiKey}`;
    
    // Highly specific system prompt to define AI persona and enforce output constraints.
...
Make it conversational and friendly, like advice from a wise friend.`;
    ...
    // Configuration payload for the API request.
    const payload = {
      contents: [{ parts: [{ text: prompt }] }],
      generationConfig: {
        // Enforce JSON output for reliable client-side parsing.
        responseMimeType: "application/json",
        responseSchema: {
          // Define the expected output structure: an array of strings.
          type: "ARRAY",
          items: { type: "STRING" }
        },
        temperature: 0.8, // Higher temperature for more creative, varied advice.
        maxOutputTokens: 300, // Limit tokens to prevent overly long responses.
      }
    };
    ...
    try {
      ...
      // Extract the raw JSON text from the nested response path.
  ...
  // Dynamically find the icon associated with the currently selected question.
  const selectedIcon = questions.find(q => q.text === selectedQuestion)?.icon || MessageCircle;
  // Component names must be capitalized for JSX rendering.
  const SelectedIcon = selectedIcon;

  return (
    // Styling the card with a rose-pink theme.
      ...
      <div className="flex flex-col text-center space-y-4">
        ...
        <div className="pt-2 space-y-4">
          ...
          <Button
            onClick={getAdvice}
            disabled={loading}
            // Use gradient for button styling to match the theme.
            className="w-full bg-gradient-to-r from-pink-500 to-rose-500 hover:from-pink-600 hover:to-rose-600"
          >
            {/* Conditional display of loading spinner vs. icon */}
            {loading ? (
        ...
        <div className="pt-4 text-left space-y-3 min-h-[140px]">
          ...
          {/* Advice List: Only render when results exist and not loading. */}
          {!loading && advice.length > 0 && (
            <div className="space-y-3">
              {/* Header displaying the icon corresponding to the question. */}
              ...
              <ul className="list-none space-y-3">
                {advice.map((tip, index) => (
                  // Distinct styling for each advice tip.
                  <li key={index} className="text-sm text-gray-800 bg-gradient-to-r from-pink-50 to-rose-50 border-l-4 border-rose-400 p-3 rounded-r-lg flex items-start gap-2">
                    {/* Numbered bullet point for tips. */}
